...
async function main() {
  ...
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@sarkarijob.local'
  const adminPassword = process.env.ADMIN_PASSWORD || 'Admin@123'
  if (process.env.NODE_ENV === 'production' && !process.env.ADMIN_PASSWORD) {
    throw new Error('ADMIN_PASSWORD must be set when seeding a production database')
  }
  ...
  const adSlots = [
    ['header', 'Header Ad', 'all', 1],
    ['below-nav', 'Below Navigation', 'all', 2],
    ['home-inline', 'Homepage Inline', 'home', 3],
    ['sidebar', 'Post Sidebar', 'post', 4],
    ['mobile-sticky', 'Mobile Sticky', 'all', 5],
    ['detail-banner', 'Post Detail Banner', 'post', 6],
    ['footer', 'Footer Ad', 'all', 7],
    ['search-inline', 'Search Inline', 'search', 8],
    ['category-inline', 'Category Inline', 'category', 9],
    ['state-inline', 'State Inline', 'state', 10],
  ] as const
  for (const [key, name, pageType, sortOrder] of adSlots) {
    await prisma.adSlot.upsert({
      where: { key }, update: { name, pageType, sortOrder },
      create: { key, name, pageType, sortOrder, provider: 'ADSENSE' },
    })
  }

  for (const flag of [
    { key: 'email_capture', name: 'Email Capture', status: 'ENABLED' as const },
    { key: 'telegram_cta', name: 'Telegram CTA', status: 'ENABLED' as const },
    { key: 'push_notifications', name: 'Push Notifications', status: 'ROLLOUT' as const, rollout: 10 },
  ]) await prisma.featureFlag.upsert({ where: { key: flag.key }, update: flag, create: flag })

  const apiKey = process.env.DEV_PUBLIC_API_KEY || 'dev_sarkarijob_key'
  const keyHash = createHash('sha256').update(apiKey).digest('hex')
  await prisma.apiKey.upsert({
    where: { key: keyHash },
    update: { keyHash, scopes: 'posts:read,exams:read,states:read,trends:read', isActive: true },
    create: { name: 'Development API Key', key: keyHash, keyHash, scopes: 'posts:read,exams:read,states:read,trends:read', permissions: 'read', rateLimit: 1000 },
  })
