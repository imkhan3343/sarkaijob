...
from models import SourceRegistry, ScrapeRunLog
...
class ScraperEngine:
    ...
    def load_sources(self, source_slug: Optional[str] = None):
        ...
        if source_slug:
            query += f" AND slug = '{source_slug}'"
        self.sources = self.db.fetch_all(query)
    ...
    async def run_source(self, source: SourceRegistry):
        ...
        try:
            module = importlib.import_module(f"sources.{source['slug']}")
    ...
    async def run_all(self):
        self.load_sources()
        tasks = [self.run_source(s) for s in self.sources]
